...
public class ModResolver {
	...
	class UrlProcessAction extends RecursiveAction {
		...
		protected void compute() {
			...
			Log.debug(LogCategory.DISCOVERY, "Testing " + url);
			...
			if (Files.isDirectory(path)) {
				...
				if (isDevelopment && !Files.exists(fabricModJson) && !Files.exists(quiltModJson)) {
					Log.warn(LogCategory.DISCOVERY, "Adding directory " + path + " to mod classpath in development environment - workaround for Gradle splitting mods into two directories");
			...
			try {
				...
				try {
					...
					Log.warn(LogCategory.DISCOVERY, String.format("Neither a fabric nor a quilt JAR at \"%s\", ignoring", path));
			...
			for (LoaderModMetadata i : info) {
				...
				if (!added) {
					Log.debug(LogCategory.DISCOVERY, candidate.getOriginUrl() + " already present as " + candidate);
				} else {
					Log.debug(LogCategory.DISCOVERY, "Adding " + candidate.getOriginUrl() + " as " + candidate);

					List<Path> jarInJars = inMemoryCache.computeIfAbsent(candidate.getOriginUrl().toString(), (u) -> {
						Log.debug(LogCategory.DISCOVERY, "Searching for nested JARs in " + candidate);
						...
						jars.stream()
							...
							.forEach((modPath) -> {
								if (!modPath.toString().endsWith(".jar")) {
									Log.warn(LogCategory.DISCOVERY, "Found nested jar entry that didn't end with '.jar': " + modPath);
									return;
								}
								...
								if (Files.isDirectory(modPath)) {
									...
									Log.debug(LogCategory.DISCOVERY, "Found nested JAR: " + modPath);
	...
	public ModSolveResult resolve(QuiltLoaderImpl loader) throws ModResolutionException {
		...
		Log.debug(LogCategory.SOLVING, "Mod solving detection time: " + (time2 - time1) + "ms");
		Log.debug(LogCategory.SOLVING, "Mod solving time: " + (time3 - time2) + "ms");
